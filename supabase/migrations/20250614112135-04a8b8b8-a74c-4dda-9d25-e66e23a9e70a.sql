...
-- Remove the foreign key constraint that requires admin_users.user_id to exist in auth.users
-- This allows our admin system to be independent of Supabase Auth
ALTER TABLE public.admin_users DROP CONSTRAINT IF EXISTS fk_admin_users_auth_users;
...
-- Also ensure the admin_auth table doesn't have conflicting constraints
ALTER TABLE public.admin_auth DROP CONSTRAINT IF EXISTS admin_auth_user_id_fkey;
ALTER TABLE public.admin_auth DROP CONSTRAINT IF EXISTS fk_admin_auth_auth_users;

-- Update the create_admin_user_v2 function to handle the constraint removal
...
BEGIN
  ...
  -- Validate branch exists if not superadmin
  IF NOT p_is_superadmin THEN
    IF NOT EXISTS(SELECT 1 FROM public.branches WHERE id = p_branch_id AND is_active = true) THEN
      RETURN jsonb_build_object('success', false, 'error', 'Invalid or inactive branch ID provided');
    END IF;
  END IF;
  ...
  EXCEPTION
    ...
    WHEN foreign_key_violation THEN
      RETURN jsonb_build_object('success', false, 'error', 'Invalid branch ID or foreign key constraint violation');
    WHEN OTHERS THEN
