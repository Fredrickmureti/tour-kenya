...
interface EnhancedReceiptDetails {
  receipt_id: string;
  booking_id: string;
  user_id: string;
  user_email: string;
...
const ReceiptPageEnhanced: React.FC = () => {
  const { receiptId } = useParams<{ receiptId: string }>();
  const location = useLocation();
  const navigate = useNavigate();
  const [receiptDetails, setReceiptDetails] = useState<EnhancedReceiptDetails | null>(
    location.state?.receiptDetails || null
  );
  ...
  useEffect(() => {
    const fetchReceipt = async () => {
      ...
      setIsLoading(true);
      try {
        const { data, error: rpcError } = await supabase.rpc('get_receipt_details', { 
        ...
        if (data && typeof data === 'object') {
          const receipt = data as unknown as EnhancedReceiptDetails;
          console.log('Enhanced receipt details received:', receipt);
  ...
  const formatDateTime = (dateTimeString: string | undefined): string => {
    if (!dateTimeString || dateTimeString === 'undefined undefined') {
...
export default ReceiptPageEnhanced;
